...
class VideoRAG:
    ...
    def __post_init__(self):
        ...
        # Determine active mode: Gemini-only OR OpenAI + DashScope
        _gemini_key = (
            self.gemini_api_key
            or os.environ.get("GEMINI_API_KEY", "")
            or ""
        ).strip()
        _using_gemini = bool(_gemini_key)

        if not _using_gemini:
            # Legacy mode: both OpenAI and DashScope keys are mandatory
            assert self.ali_dashscope_api_key, (
                "ali_dashscope_api_key is required when gemini_api_key is not set"
            )
            assert self.ali_dashscope_base_url, (
                "ali_dashscope_base_url is required when gemini_api_key is not set"
            )
            assert self.openai_api_key, (
                "openai_api_key is required when gemini_api_key is not set"
            )
            assert self.openai_base_url, (
                "openai_base_url is required when gemini_api_key is not set"
            )
        # caption_model and asr_model may be None in Gemini mode; handled downstream
        ...
        self.safe_config = {
            ...
            "gemini_api_key": self.gemini_api_key or os.environ.get("GEMINI_API_KEY", ""),
